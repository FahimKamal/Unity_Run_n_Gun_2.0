...
public class PlayerController : MonoBehaviour
{
    ...
    void PlayerMovement()
    {
        ...
        if (groundedPlayer && !isAimModeActive && (Input.GetKeyDown(KeyCode.W) || autoJump))
        {
            autoJump = false;
        ...
        // Player Roll
        if(groundedPlayer && !isAimModeActive && (Input.GetKeyDown(KeyCode.S) || autoRoll))
        {
            autoRoll = false;
        ...
        if (!isAimModeActive && turnAvailable && Mathf.Abs(input.x) > 0f)
        {
            Turn(input.x);
        }
